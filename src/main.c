...
int SaveToFile(Board board, Player *players, int NumPlayers)
{
	FILE *fptr = fopen("GameStage.txt","w");
		fprintf(fptr,"%d %d\n", board.size.x, board.size.y);
		for(int i=0; i < (board.size.y); i++)
			{
			for(int j=0; j < (board.size.x); j++)
				{
				if(board.grid[i * board.size.x + j] >= 0)
				{fprintf(fptr,"%d0 ", board.grid[i * board.size.x + j]);}
				else
				{fprintf(fptr,"0%d ", -(board.grid[i * board.size.x + j]));}
				}
				fprintf(fptr,"\n");
			}
		for(int i=0; i < NumPlayers; i++)
			fprintf(fptr,"%s %d %d\n", players[i].Name, i+1, players[i].Score);
		fclose(fptr);
				...
				case BtnPressedEvent:
				{
					...
					case BtnNext_id:
					{
						...
						if(buff[0]!='\0' && ColorSet){
							...
							step(&Stage, &players,&CurrentPlayerNum, &CurrentPenguinNum, NumPlayers,NumPengPerPlayer,&AskingHWNDs.MainLabel,NULL, NULL);
							HDC memDC = CreateCompatibleDC(dc);
							HBITMAP memBM = CreateCompatibleBitmap(dc, 102,102);
							SelectObject(memDC, memBM);
							SelectObject(memDC, GetStockObject(DC_BRUSH));
							...
							case Placement:
								{
									...
									if(collum>=0 && row>=0&&collum<board.size.x&&row<board.size.y){
										...
										if(errorCode == 1){
											...
											SaveToFile(board,  players, NumPlayers);
										}else{
